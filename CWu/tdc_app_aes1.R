                           ...
                           sidebarLayout(sidebarPanel(
                             fileInput('file','File input',
                                       accept=c('text/csv',
                                                ...
                                                '.csv')),
                             br(),
                             h4("Or use the default data"),
                             selectInput("default_data", "Select Default Data", c("None","bilidata"), selected = "bilidata")
                           ),
                           mainPanel(
...
server <- function(input, output) {
  ...
  default_data <- reactive({
    if (input$default_data == "bilidata") {
      read.csv("www/bilidata.csv")
    } else {
      NULL
    }
  })
